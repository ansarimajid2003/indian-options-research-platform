...
*Updated 2026-04-30: broker_sim.py patched (2026 STT, spread proxy), download_nse_bhavcopy.py and scrape_nse_option_chain.py written. VIX gaps confirmed filled (cleaned_archive covers 2015–April 2026 daily + 15-min).*
...
**How to fill — cheap:**
- **Dhan expired options API** (₹0 with a funded Dhan account) — 5 years of 1-min OHLCV+OI+IV for expired F&O contracts via `api.dhan.co/v2/charts/intraday`. Note: poll in 90-day windows (API limit per call). Already integrated in `download_dhan_expired_options.py`. Activate and run. This is the highest-priority extension — it adds 5 years of intraday option data for free.
...
- This analysis costs nothing — we already have the spot data in `cleaned_archive/NIFTY 50_15minute.csv`. It is a 30-minute coding task in `analyze_3pm_edge_v2.py`.
...
| NSE F&O Bhavcopy archives (`nseindia.com/all-reports`) | EOD OHLC, volume, OI, bid/ask for all options | Free | 2008–present | **BUILT** — run `python download_nse_bhavcopy.py --from-date 2015-01-01` |
| NSE India VIX historical (`nseindia.com/reports-indices-historical-vix`) | Daily VIX OHLC | Free | 2008–present | **ALREADY HAVE** — `cleaned_archive/INDIA VIX_day.csv` covers 2015–2026 |
| Dhan API (`api.dhan.co/v2/charts/intraday`) | 1-min OHLCV+OI+IV for expired F&O, 5 years | Free (funded account) | ~5 years back | Highest — activate `download_dhan_expired_options.py` (needs DHAN_ACCESS_TOKEN) |
...
| Yahoo Finance `^INDIAVIX` | Daily VIX OHLC | Free | Multi-year | **SKIP** — `cleaned_archive/INDIA VIX_day.csv` already covers this |
...
12. **Run `download_dhan_expired_options.py`** to extend option history. Cross-validate close prices against Shoonya on overlapping dates.
