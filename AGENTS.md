...
Phase: exit window validation → structure upgrade (debit spreads).
...
**All three engine bugs are fixed (May 2026).** Re-run all strategies with the updated cost model before drawing conclusions.
...
1. ~~**Fix three bugs** (expiry, calendar, stop gate) + update STT in `broker_sim.py`~~ ✓ Done May 2026. Also: tiered slippage model, OI liquidity thresholds, asymmetric exit fills, Sortino/Calmar metrics.
...
- **Expiry regime break:** NSE changed NIFTY weekly expiry Thursday → Tuesday on 2025-09-02. `calendar.py` handles this. Any backtest spanning this date mixes two regimes — `summary()` flags `regime_break_in_sample: true`.
